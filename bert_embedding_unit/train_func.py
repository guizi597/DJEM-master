...
def generate_candidate_dict(Model, train_ent1s, train_ent2s, for_candidate_ent1s, for_candidate_ent2s,
    ...
    """
    generate candidate dictionaries for entities
    """
...
def train(Model, Criterion, Optimizer, Train_gene, train_ill, test_ill, entid2data):
    ...
    for epoch in range(EPOCH_NUM):
        ...
        Optimizer.zero_grad()  # initialization gradient
...
def test(Model, ent_ill, entid2data, batch_size, context=""):
    ...
    start_time = time.time()
    print(context)
    Model.eval()  # follow the value of batch normalization instead of dropout
    with torch.no_grad():  # a context manager, the part wrapped by the statement will not track
...
def save(Model, train_ill, test_ill, entid2data, epoch_num):
    """
    save model and entity embedding
    """
...
def ent_align_train(Model, Criterion, Optimizer, Train_gene, entid2data):
    """
    return loss of training entity alignment
    """
    start_time = time.time()
    all_loss = 0
    ...
    for pe1s, pe2s, ne1s, ne2s in Train_gene:
        ...
        neg_score = F.pairwise_distance(neg_emb1, neg_emb2, p=1, keepdim=True)  # L1 distance
        del neg_emb1
        del neg_emb2

        label_y = -torch.ones(pos_score.shape).cuda(CUDA_NUM)  # pos_score < neg_score
        batch_loss = Criterion(pos_score, neg_score, label_y)  # calculate loss
        del pos_score
        del neg_score
        del label_y

        batch_loss.backward()  # back propagation calculation gradient
