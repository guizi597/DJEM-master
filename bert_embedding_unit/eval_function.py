...
def batch_topk(mat, bs=128, topn=50, largest=False, cuda_num=0):
    """
    returns the result score and result index
    """
...
def hit_res(index_mat):
    """
    returns the result of the evaluation indicator(HitRatio@[1-50])
    """
    ...
    index_list = []
    index_list.append(topk_n[1-1])
    index_list.append(topk_n[10 - 1])
    index_list.append(topk_n[50 - 1])
    index_list.append(MRR)
    pickle.dump(index_list, open(MODEL_SAVE_PATH + MODEL_SAVE_PREFIX + 'bert_index.pkl', "wb"))